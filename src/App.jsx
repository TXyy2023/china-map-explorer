...
const SITE_THEME_OPTIONS = [
  ...
  { id: 'crimson-palace', label: '宫红', notice: '系统视觉已切换为：宫红宴席风' },
  { id: 'porcelain-blue', label: '青瓷', notice: '系统视觉已切换为：青瓷雅集风' },
  { id: 'market-night', label: '夜市', notice: '系统视觉已切换为：夜市烟火风' },
];
...
const mapMachine = createMachine({
  ...
  context: {
    ...
    selectedAreaId: 'sichuan-chongqing-food', // 当前聚焦的美食区域 ID
  },
  on: {
    // 动作事件：返回全国视图
    BACK_COUNTRY: {
...
function normalizeFoodItems(area) {
  ...
  const items = configuredItems?.length
    ...
    : (area.assets || []).map((asset) => ({
        ...
        process: ['备好地方食材', '以本地调味打底', '慢火成味', '装盘趁热分享'],
      }));
  ...
  return items.map((item, index) => ({
    ...
    name: item.name || item.title || `美食 ${index + 1}`,
    objectPosition: item.objectPosition || '50% 50%',
    process: item.process?.length ? item.process : ['备料切配', '热锅成香', '调味收口', '装盘上桌'],
...
function buildFoodItemLayout(items, mapSize, focusBounds, viewLevel, areaId) {
  ...
  const baseSize = clampNumber(Math.min(width, height) * 0.135, 62, 92);
  const safeX = baseSize / 2 + 12;
  const safeTop = baseSize / 2 + 14;
  const safeBottom = baseSize / 2 + 62;
  ...
  for (let index = 0; index < ringCount; index += 1) {
    ...
    const radiusX = Math.max(focus.w * 0.5 + baseSize * 0.8, width * 0.18) * radiusJitter;
    const radiusY = Math.max(focus.h * 0.5 + baseSize * 0.78, height * 0.2) * radiusJitter;
    pushCandidate(
...
function FoodMap({
  ...
  return (
    <div className="map-stage" ref={mapStageRef}>
      ...
      >
        ...
        {mapLoading ? (
          ...
          <>
            ...
            {/* 2. 上层： bounds 实时物理裁切美食大图，通过 pointer-events: none 将鼠标事件完美穿透给底层交互 */}
            {viewLevel === 'country' && chinaBounds && (
              <image
            ...
            {viewLevel === 'area' && areaBounds && currentArea && (
              <image
            ...
            {viewLevel === 'province' && currentArea && (
              <>
                ...
                {/* 2.2 聚焦激活的省份以 100% 不透明度呈现省级特色美食图 */}
                {(() => {
      ...
      {activeFoodArea && foodLayoutItems.length > 0 && (
        <div className={`food-orbit-layer ${isFoodLayerReady ? 'is-ready' : ''}`} aria-label={`${activeFoodArea.name}美食小图`}>
          {foodLayoutItems.map((item) => {
            const isSelected = selectedFoodItem?.id === item.id && selectedFoodItem?.areaId === activeFoodArea.id;
            return (
              <button
                aria-label={`查看${item.name}制作流程`}
 ...
 */
function RunSummary({ assetVersions, currentArea, selectedFoodItem, selectedProvinceAdcode, viewLevel, onBackCountry, onBackArea, theme }) {
  // 分数统计及答题闯关状态
  ...
  return (
    <aside className="side-card run-card">
      ...
      <p>
        {viewLevel === 'province' && activeAsset
          ? (activeAsset.quiz?.question ? `欢迎来到${activeAsset.title.replace('图', '')}！在这里，我们用AI精细描绘了最具代表性的本土传统风味，邀您进行趣味特产答题挑战。` : '探索省级独特美食文化。')
          : (viewLevel === 'area' && currentArea)
        ...
        {viewLevel === 'country' ? (
          <>
            <span style={{ color: '#d4af37', borderColor: '#d4af3740', background: '#d4af3710' }}>
              {theme.areas.length} 个美食文化区
            </span>
            <span>
              {theme.areas.reduce((acc, a) => acc + (a.assets?.length || 0), 0)} 张 AI 灵感图片
            </span>
          </>
        ) : (
          <>
            ...
            <span>{currentArea?.assets?.length || 0} 张 AI 灵感图片</span>
          </>
        )}
      </div>

      {viewLevel !== 'country' && selectedFoodItem && (
        <section className="food-process-card" key={`${selectedFoodItem.areaId}-${selectedFoodItem.id}`}>
        ...
        <div className="game-container">
          <div className="game-title">
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span>
                {viewLevel === 'province' && activeAsset ? `${activeAsset.title.substring(0, 3)}·特产闯关` : '美食大区答题闯关'}
              </span>
            </div>
...
function McpPanel({ assetVersions, currentArea, flowStep, onAssetRegenerated, onFlowStep, onSelectArea, onToolResult, theme }) {
  ...
  async function generatePrompt(values) {
    ...
    onFlowStep('prompt');
    onToolResult('AI 灵感舱成功编译：美食绘图 Prompt 已就绪');
  }
  ...
  return (
    <aside className="side-card mcp-panel">
      ...
      <div className="prompt-box">
        <strong>{prompt ? '编译生成的 AI 绘图词' : '当前地图图片资产'}</strong>
...
/**
 * 华夏美食文化交互地图大盘——React 核心主入口组件
 */
function App() {
  const [notice, setNotice] = useState('华夏美食文化地图已就绪，等待探索');
  const [assetVersions, setAssetVersions] = useState({});
  const [siteTheme, setSiteTheme] = useState('dark-ink'); // 全局视觉主题：见 SITE_THEME_OPTIONS
  const [hoveredAreaId, setHoveredAreaId] = useState('sichuan-chongqing-food');
  ...
  function handleFoodItemSelect(item, areaId) {
    ...
    if (viewLevel === 'country') {
      setSelectedProvinceAdcode(null);
      setViewLevel('area');
      setNotice(`已进入${area.name}，展开「${nextFoodItem.name}」制作流程`);
      return;
    }
    ...
    setNotice(`「${nextFoodItem.name}」制作流程已展开`);
  }
      ...
      <section className="workspace">
        ...
        <div className="map-column">
          <div className="map-title-row">
            <div>
              <span>华夏美食地理大探索 ✦ React 自适应投影</span>
              <h1>
                {viewLevel === 'province'
          ...
          <div className="side-card notice-card">
            ...
            <button
              ...
              onClick={() => {
                ...
                setSelectedProvinceAdcode(null);
                setSelectedFoodItem(null);
                setHoveredAreaId('sichuan-chongqing-food');
                setViewLevel('country');
                send({ type: 'BACK_COUNTRY' });
                setNotice('系统成功复位：地图已恢复至默认华夏美食配置');
              }}
              type="button"
            >
